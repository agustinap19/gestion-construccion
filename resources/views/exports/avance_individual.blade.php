...
<h1>Reporte de Avance Individual — {{ $vivienda->beneficiario ? $vivienda->beneficiario->nombre . ' ' . $vivienda->beneficiario->apellido_paterno : 'Sin Asignar' }}</h1>
<div class="subtitle">
...
<div class="kpi-row">
    <div class="kpi">
        <div class="kpi-val">{{ number_format($vivienda->porcentaje_avance, 1) }}%</div>
        <div class="kpi-lbl">Avance Físico</div>
    </div>
    <div class="kpi">
        <div class="kpi-val">{{ $vivienda->itemsChecklist->where('estado','completado')->count() }}/{{ $vivienda->itemsChecklist->count() }}</div>
        <div class="kpi-lbl">Items Completados</div>
    </div>
    <div class="kpi">
        <div class="kpi-val">{{ $reportes->count() }}</div>
        <div class="kpi-lbl">Últimos Reportes</div>
    </div>
    <div class="kpi">
        <div class="kpi-val">{{ ucfirst(str_replace('_', ' ', $vivienda->estado)) }}</div>
