...
<body>
    <div class="container">
        ...
        @if($valido)
            ...
            <div class="details">
                <div class="detail-row">
                    ...
                    <span class="detail-value">{{ strtoupper(str_replace('_', ' ', $documento->tipo_reporte)) }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Fecha de Emisión:</span>
                    <span class="detail-value">{{ $documento->fecha_emision->format('d/m/Y H:i:s') }}</span>
                </div>
                <div class="detail-row">
                    <span class="detail-label">Emitido por:</span>
                    <span class="detail-value">{{ $documento->emisor->nombre ?? 'Sistema' }} {{ $documento->emisor->apellido_paterno ?? '' }}</span>
                </div>
                @if($documento->proyecto)
                <div class="detail-row">
                    <span class="detail-label">Proyecto:</span>
                    <span class="detail-value">{{ $documento->proyecto->nombre }}</span>
                </div>
                @endif
                <div class="detail-row" style="border-bottom: none;">
                    <span class="detail-label">Hash Único:</span>
                    <span class="detail-value" style="font-size: 11px; word-break: break-all; text-align: left; margin-left: 10px;">{{ $documento->hash }}</span>
                </div>
            </div>
        @else
            <div style="text-align: center; margin-bottom: 20px;">
        ...
        <div class="footer">
            &copy; {{ date('Y') }} CA & KANAGF S.R.L. Todos los derechos reservados.
        </div>
    </div>
</body>
</html>
