...
import {
    ...
    CheckCircle, Lock, AlertTriangle, Users, History,
...
// ── Foto/avatar de la fila ──────────────────────────────────────────────────────
function ActivoFoto({ a }) {
...
// ── Row ───────────────────────────────────────────────────────────────────────
function ActivoRow({ a, onVer, onEdit, onEliminar, canEditar, canEliminar }) {
    return (
        <motion.tr initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <td className="px-4 py-3">
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    {canEliminar && (
                        <button onClick={e => { e.stopPropagation(); onEliminar(a); }}
...
export default function ActivosIndex() {
    ...
    const canVerActas = hasPermission('activos.ver_actas');
    ...
    const cargar = useCallback(async (page = 1) => {
        ...
        try {
            const [listRes, statsRes] = await Promise.all([
                ...
                activoService.estadisticas(),
            ]);
            const paginador = listRes.data ?? {};
        ...
        <div className="min-h-screen p-6 space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                ...
                <div className="flex items-center gap-2">
                    {canVerActas && (
                        <button onClick={() => navigate('/dashboard/activos/historial-entregas')}
                            className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/[0.06] border border-white/10 text-white/70 text-sm font-medium hover:bg-white/10 hover:text-white transition-all">
                            <History className="w-4 h-4" /> Entregas finalizadas
                        </button>
                    )}
                    <BotonExportar
                        url="/exportar/activos"
                        filtros={{
                            buscar: filtros.buscar || undefined,
                            tipo: filtros.tipo === 'todos' ? undefined : filtros.tipo,
                            estado: filtros.estado === 'todos' ? undefined : filtros.estado,
            ...
            {stats && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <KpiCard label="Disponibles" value={stats.disponibles} icon={CheckCircle} color="bg-emerald-500/20 text-emerald-300" />
                    <KpiCard label="En mantenimiento" value={stats.mantenimiento} icon={AlertTriangle} color="bg-amber-500/20 text-amber-300" />
                    <KpiCard label="Costo/día total" value={`Bs. ${Number(stats.valorDiaTotal ?? 0).toFixed(0)}`} icon={Lock} color="bg-blue-500/20 text-blue-300" />
                </div>
            )}
                                ...
                                <AnimatePresence>
                                    {activos.map(a => (
                                        <ActivoRow key={a.id} a={a}
                                            onVer={av => navigate(`/dashboard/activos/${av.id}/asignaciones`)}
